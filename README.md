...
| `brainstorming` | `ブレスト`, `設計したい`, `作りたい`, `brainstorm`, `設計`; proactive (start of non-trivial implementation work) | Force a requirements → design dialogue before any implementation. Fixes "what to build" and "acceptance criteria", writes a design doc, picks an implementation style (4 combinations of isolation × parallelism), then commits the spec on a fresh branch/worktree before handing off to `writing-plans`. In **autonomous** mode (see `cross-review`) it skips clarifying questions and user approval — assumptions are written into the spec and the spec is approved by a cross-model review instead. **Never commits to main / default branch directly.** |
...
| `subagent-driven-development` | `subagent driven`, `SDD`, `subagent 駆動`, `plan を消化`, `plan を実行` | Execute a `writing-plans` plan inside the current session by dispatching a fresh implementer subagent per task, gating each with a task reviewer (spec compliance + code quality), and running one whole-branch reviewer at the end. Each subagent receives only the context the controller selects (no session-history pollution), keeping both the implementer focused and the controller's coordination context preserved. Persists progress to `.claude/sdd/progress.md` so a compaction-killed session can resume without re-dispatching completed tasks; passes briefs / reports / diffs as files instead of pasting into prompts. Includes a pre-flight gate that batches plan ↔ review-rule conflicts and downstream-prereq misses into a single adjudication before Task 1 dispatch — asked to the user in guarded mode, or ruled by a different Claude model via `cross-review` in autonomous mode. Stops only before `Gate: human` tasks and at the end, leaving the work as commits on the feature branch. |
| `cross-review` | `cross-review`, `クロスレビュー`, `別モデルでレビュー`, `自律モード`, `慎重モード`; referenced by `brainstorming` / `writing-plans` / `subagent-driven-development` | Autonomy policy + cross-model review gate for the brainstorming → writing-plans → SDD pipeline. Decides **autonomous** vs **guarded** mode (explicit user wording > project `.claude/rules/autonomy.md` > risk escalation for prod data / migrations / auth / billing / PII / deploy / external sends > default: autonomous when Auto Mode is on). In autonomous mode, human approval gates are replaced by a review from a **different Claude model** given an explicit alternative perspective (requester's proxy for specs, zero-context implementer for plans, neutral arbiter for adjudication, maintainer + attacker for the whole branch), with a 3-round fix loop, `Needs Human` only for irreversible / undecidable points, an audit log in the spec, per-task `Gate: human` stops, and a hard stop at a commit on the feature branch (no push / PR / merge). |
...
| `brainstorming` | `ブレスト` / `設計したい` / `作りたい` / `brainstorm` / `設計`; proactive（非自明な実装着手時） | 実装着手前に「要件 → 設計」の対話を強制し、「何を作るか / 受け入れ条件」を確定。design doc 書き出し → 実装スタイル決定（隔離 × 並列の 4 択）→ branch / worktree を切ってから spec を commit → `writing-plans` に引き継ぐまでが責務。**main / 既定 branch への直 commit は禁止**。**autonomous** モード（`cross-review` 参照）では明確化質問とユーザー承認を行わず、仮定を spec に明記したうえで別モデルのレビューで承認する。 |
...
| `subagent-driven-development` | `subagent driven` / `SDD` / `subagent 駆動` / `plan を消化` / `plan を実行` | `writing-plans` で書いた実装プランを **タスクごとに fresh implementer subagent** を派遣して同セッション内で消化する。各タスク完了後に **task reviewer**（spec 適合 + コード品質）、全タスク終了後に **whole-branch reviewer** を 1 回回す。subagent は controller が選んだ context だけを受け取り session 履歴を引き継がないため、implementer は集中して succeed し controller の context も coordination 用に温存される。完了状況は `.claude/sdd/progress.md` の ledger で永続化し、compaction で履歴が消えても完了済み task を再派遣せず resume できる。brief / report / diff は dispatch prompt に貼らず file 渡しにして context 汚染を防ぐ。Task 1 派遣前の pre-flight gate で plan ↔ review 規約の矛盾と下流前提の不在を 1 件にまとめて adjudicate する（guarded はユーザーに質問、autonomous は `cross-review` で別 Claude モデルが裁定）。停止するのは `Gate: human` タスクの直前と完了時のみで、成果は feature branch 上の commit として残す。 |
| `cross-review` | `cross-review` / `クロスレビュー` / `別モデルでレビュー` / `自律モード` / `慎重モード`; `brainstorming` / `writing-plans` / `subagent-driven-development` から参照 | 新規開発パイプラインの自律モード判定と別モデルレビューゲート。**autonomous / guarded** をユーザー発話 > PJ の `.claude/rules/autonomy.md` > リスク昇格 (本番データ / migration / 認証 / 課金 / 個人情報 / デプロイ / 外部送信) > 既定 (Auto Mode 有効なら autonomous) の順で決める。autonomous では人間の承認ゲートを **セッションと異なる Claude モデル** による視点指定付きレビュー (spec は依頼者の代理人、plan はゼロ context の実装者、裁定は中立な裁定者、最終は保守担当 + 攻撃者) に置き換える。修正ループは最大 3 往復、`Needs Human` は不可逆 / 決定不能な論点に限定、判断は spec の自律判断ログに記録、`Gate: human` タスクの直前では停止、終点は feature branch 上の commit (push / PR / merge はしない)。 |
