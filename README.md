...
| `brainstorming` | `ブレスト`, `設計したい`, `作りたい`, `brainstorm`, `設計`; proactive (start of non-trivial implementation work) | For non-trivial work involving design decisions (trivial fixes such as a typo or a config value are out of scope), run a requirements → design dialogue before implementation. Fixes "what to build" and "acceptance criteria", writes a design doc, picks an implementation style (4 combinations of isolation × parallelism), then commits the spec on a fresh branch/worktree before handing off to `writing-plans`. In **autonomous** mode (see `cross-review`) it skips clarifying questions and user approval — assumptions are written into the spec and the spec is approved by an alternative-perspective review instead. **Never commits to main / default branch directly.** |
...
| `subagent-driven-development` | `subagent driven`, `SDD`, `subagent 駆動`, `plan を消化`, `plan を実行` | Execute a `writing-plans` plan inside the current session by dispatching a fresh implementer subagent per task, gating each with a task reviewer (spec compliance + code quality), and running one whole-branch reviewer at the end. Each subagent receives only the context the controller selects (no session-history pollution), keeping both the implementer focused and the controller's coordination context preserved. Persists progress to `.claude/sdd/progress.md` so a compaction-killed session can resume without re-dispatching completed tasks; passes briefs / reports / diffs as files instead of pasting into prompts. Includes a pre-flight gate that batches plan ↔ review-rule conflicts and downstream-prereq misses into a single adjudication before Task 1 dispatch — asked to the user in guarded mode, or ruled by a fresh reviewer subagent via `cross-review` in autonomous mode. Stops only before `Gate: human` tasks and at the end, leaving the work as commits on the feature branch. |
| `cross-review` | `cross-review`, `クロスレビュー`, `別視点でレビュー`, `自律モード`, `慎重モード`; referenced by `brainstorming` / `writing-plans` / `subagent-driven-development` | Autonomy policy + alternative-perspective review gate for the brainstorming → writing-plans → SDD pipeline. Decides **autonomous** vs **guarded** mode (explicit user wording > project `.claude/rules/autonomy.md` > risk escalation for prod data / migrations / auth / billing / PII / deploy / external sends > default: autonomous when Auto Mode is on). In autonomous mode, human approval gates are replaced by a review from a **fresh subagent** given an explicit alternative perspective (same model by default; a different Claude model is optional) (requester's proxy for specs, zero-context implementer for plans, neutral arbiter for adjudication, maintainer + attacker for the whole branch), with a 3-round fix loop, `Needs Human` only for irreversible / undecidable points, an audit log in the spec, per-task `Gate: human` stops, and a hard stop at a commit on the feature branch (no push / PR / merge). |
...
| `crit-review` | `○○をレビューして`, `AIレビュー`, `LLMレビュー`, `crit-review` | Have an LLM reviewer — Fable / Opus / Sonnet (fresh subagent with model override) or Codex (`codex exec`, no subagent), chosen via AskUserQuestion — review files, launch the crit UI in the background, publish the findings into the live session as crit inline comments (`crit comment --json --file`, author = model name), and wait for Finish Review to address the user's replies. Reverse direction of the official `/crit` skill: the LLM is the reviewer, the human reads and replies. Falls back to plain `file:line` chat output when the user opts out of crit. **Requires the `crit` CLI; `codex` CLI needed only for Codex reviews.** |
...
| `cognitive-rhythm-writing` | `認知リズム`, `緩急をつけて`, `平坦で退屈`, `リズムを設計`; proactive (writing prose meant to be read as engaging) | Japanese writing rules for designing cognitive rhythm (緩急) in explanatory prose — treating pacing not as decoration but as switching of the reader's cognitive modes (observe → waver → assert → re-examine) and management of unresolved tension. Covers sentence beats, paragraph density waveforms, section openings, telling slack from filler ("does it update the situation or the document?"), and a mechanical post-writing inspection procedure. Companion layer on top of `japanese-tech-writing` (reads `../japanese-tech-writing/SKILL.md` first — install both). Borrowed verbatim from [k16shikano's gist](https://gist.github.com/k16shikano/eb2929f13ed19c97188393d297be8432); the author [declares the Unlicense for all their public gists](https://gist.github.com/k16shikano/67625f2a7d96e3bbdfae8d571a936063) (full text bundled as `LICENSE` in the skill directory). |
| `herdr-fork` | `herdr-fork`, `セッションフォーク`, `別ペインでフォーク` | Fork the current Claude Code session into a new Herdr pane or tab (`right` / `down` / `tab`) via `claude --resume <sid> --fork-session` — the fork inherits the full conversation history under a new session ID and the original session stays untouched. Based on [miyagawa's gist](https://gist.github.com/miyagawa/cb1a9f6c8695d1219efba0c66d5f78f7), extended with a `tab` direction (focuses the new tab) and a server-reachability guard so it also works from Herdr shell keybindings. **Requires Herdr and `python3`.** |
| `herdr-fork-popup` | `herdr-fork-popup`, `popupでフォーク`, `ポップアップでフォーク` | Fork the current Claude Code session into a Herdr session-modal popup terminal (80% x 80%) without touching the tiled layout. Herdr popups can only be opened as plugin panes, so a minimal Herdr plugin (`trapple.herdr-fork-popup`) is bundled; one-time `herdr plugin link <skill>/plugin` required. **Requires Herdr 0.7.4+ and `python3`.** |
...
| `brainstorming` | `ブレスト` / `設計したい` / `作りたい` / `brainstorm` / `設計`; proactive（非自明な実装着手時） | 設計判断を伴う非自明な変更で (typo や設定値変更など自明な修正は対象外)、実装着手前に「要件 → 設計」の対話を行い、「何を作るか / 受け入れ条件」を確定。design doc 書き出し → 実装スタイル決定（隔離 × 並列の 4 択）→ branch / worktree を切ってから spec を commit → `writing-plans` に引き継ぐまでが責務。**main / 既定 branch への直 commit は禁止**。**autonomous** モード（`cross-review` 参照）では明確化質問とユーザー承認を行わず、仮定を spec に明記したうえで別モデルのレビューで承認する。 |
...
| `subagent-driven-development` | `subagent driven` / `SDD` / `subagent 駆動` / `plan を消化` / `plan を実行` | `writing-plans` で書いた実装プランを **タスクごとに fresh implementer subagent** を派遣して同セッション内で消化する。各タスク完了後に **task reviewer**（spec 適合 + コード品質）、全タスク終了後に **whole-branch reviewer** を 1 回回す。subagent は controller が選んだ context だけを受け取り session 履歴を引き継がないため、implementer は集中して succeed し controller の context も coordination 用に温存される。完了状況は `.claude/sdd/progress.md` の ledger で永続化し、compaction で履歴が消えても完了済み task を再派遣せず resume できる。brief / report / diff は dispatch prompt に貼らず file 渡しにして context 汚染を防ぐ。Task 1 派遣前の pre-flight gate で plan ↔ review 規約の矛盾と下流前提の不在を 1 件にまとめて adjudicate する（guarded はユーザーに質問、autonomous は `cross-review` のレビュアー subagent が裁定）。停止するのは `Gate: human` タスクの直前と完了時のみで、成果は feature branch 上の commit として残す。 |
| `cross-review` | `cross-review` / `クロスレビュー` / `別視点でレビュー` / `自律モード` / `慎重モード`; `brainstorming` / `writing-plans` / `subagent-driven-development` から参照 | 新規開発パイプラインの自律モード判定と別視点レビューゲート。**autonomous / guarded** をユーザー発話 > PJ の `.claude/rules/autonomy.md` > リスク昇格 (本番データ / migration / 認証 / 課金 / 個人情報 / デプロイ / 外部送信) > 既定 (Auto Mode 有効なら autonomous) の順で決める。autonomous では人間の承認ゲートを **新しい subagent** による視点指定付きレビュー (モデルは既定でセッションと同じ、別モデルは任意) (spec は依頼者の代理人、plan はゼロ context の実装者、裁定は中立な裁定者、最終は保守担当 + 攻撃者) に置き換える。修正ループは最大 3 往復、`Needs Human` は不可逆 / 決定不能な論点に限定、判断は spec の自律判断ログに記録、`Gate: human` タスクの直前では停止、終点は feature branch 上の commit (push / PR / merge はしない)。 |
...
| `crit-review` | `○○をレビューして` / `AIレビュー` / `LLMレビュー` / `crit-review` | 選択したモデル — Fable / Opus / Sonnet（model override した fresh subagent）または Codex（`codex exec` 直接実行）を AskUserQuestion で確認 — にファイルをレビューさせ、crit UI をバックグラウンド起動 → 実行中セッションに指摘をインラインコメントとして一括投稿（`crit comment --json --file`、author = モデル名）→ Finish Review まで待機してユーザーの返信に対応する。公式 `/crit`（人間がレビュアー）の**逆方向**で、LLM がレビュアー・人間が閲覧側。crit を使わない選択をした場合は `file:line` 参照つきでチャットに整形出力。**`crit` CLI 必須 / Codex 選択時のみ `codex` CLI 必要**。 |
...
| `cognitive-rhythm-writing` | `認知リズム` / `緩急をつけて` / `平坦で退屈` / `リズムを設計`; proactive（読み物として読ませたい文章の執筆時） | 説明的な文章に緩急（認知リズム）を設計するための規範。緩急を装飾ではなく認知モードの切替（観察→逡巡→断定→再観察）と未回収の緊張の管理として扱い、文の拍 / 段落の密度波形 / 節の入り方 / 緩みと駄文の見分け方（「状況更新か、文書更新か」）/ 執筆後の機械的な点検手順 を定める。`japanese-tech-writing` の上に載せる併用レイヤー（冒頭で `../japanese-tech-writing/SKILL.md` を参照するため両方インストールする）。規範本文は [k16shikano 氏の gist](https://gist.github.com/k16shikano/eb2929f13ed19c97188393d297be8432) から拝借（著者は [public gist 全体への Unlicense 適用を宣言](https://gist.github.com/k16shikano/67625f2a7d96e3bbdfae8d571a936063)。スキルディレクトリに Unlicense 全文を `LICENSE` として同梱）。 |
| `herdr-fork` | `herdr-fork` / `セッションフォーク` / `別ペインでフォーク` | 現在の Claude Code セッションを `claude --resume <sid> --fork-session` で herdr の新しいペイン / タブ（`right` / `down` / `tab`）にフォークする。フォークは履歴を全部引き継いだ新しいセッションIDで動き、元セッションは無変更。[miyagawa 氏の gist](https://gist.github.com/miyagawa/cb1a9f6c8695d1219efba0c66d5f78f7) をベースに、`tab` 方向（新タブへフォーカス移動）と herdr キーバインドからの起動対応（サーバー到達性ガード）を拡張。**herdr と `python3` 必須。** |
| `herdr-fork-popup` | `herdr-fork-popup` / `popupでフォーク` / `ポップアップでフォーク` | 現在の Claude Code セッションを herdr のセッションモーダルなポップアップ（80%×80%）にフォークする。タイルレイアウトは変更しない。herdr の popup はプラグインペインとしてのみ開けるため最小プラグイン（`trapple.herdr-fork-popup`）を同梱し、初回のみ `herdr plugin link <skill>/plugin` が必要。**herdr 0.7.4+ と `python3` 必須。** |
...
例外: `japanese-tech-writing` と `cognitive-rhythm-writing` の規範本文は [k16shikano 氏の gist](https://gist.github.com/k16shikano) 由来（著者は [public gist 全体への Unlicense 適用を宣言](https://gist.github.com/k16shikano/67625f2a7d96e3bbdfae8d571a936063)）。詳細は各スキルディレクトリの `LICENSE` を参照。
