...
| `user-journey` | `要件確認`, `ジャーニー検索`, `user_journey`, `ユーザージャーニー`, `○○の要件は`, `○○の方針は`, `user-journey`; proactive (design / impl start) | A keyword-searchable knowledge base of a project's important requirements / constraints / rules, accumulated as short one-liners in a SQLite DB. Search before designing or implementing to stay consistent with what the user has previously stated; append new rules only with explicit permission (history-preserving INSERT, with a 4-choice flow when a new rule conflicts with an existing record). **Requires the `sqlite3` CLI; assumes a project-local `data/user_journey.db`.** |
...
| `user-journey` | `要件確認` / `ジャーニー検索` / `user_journey` / `ユーザージャーニー` / `○○の要件は` / `○○の方針は` / `user-journey` / proactive（設計・実装着手時） | プロジェクトの重要な要件・制約・ルールを SQLite に短文で蓄積し、キーワード検索できるナレッジベース。設計・実装に着手する前に検索して過去のユーザー表明と整合を取り、新ルールは許可を得てから追記する（履歴保持 INSERT、既存と矛盾するときは 4 択フローで取り扱いを確認）。他文書からは `user_journey id N` 表記で引用。**要 `sqlite3` CLI / プロジェクトローカルに `data/user_journey.db` を作る運用前提**。 |
